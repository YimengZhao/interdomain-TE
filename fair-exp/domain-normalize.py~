...
def TE():
    ...
    for key, value in trafficMatrix_1.iteritems():
        trafficMatrix_2.update({key: 10 * value})
    ...
    maxmin_fair_allocate(trafficClasses_1, linkcaps, pptc_1)

    pptc_2 = initOptimization(ie_path_map_2, topo, trafficClasses_2)
    maxmin_fair_allocate(trafficClasses_2, linkcaps, pptc_2)
    ...
    maxmin_fair_allocate(g1.keys(), linkcaps, g1)
    maxmin_fair_allocate(g2.keys(), linkcaps, g2)
    ...
    u = (netstat_1 + netstat_2 ) / 2
    print u
    gfi = math.sqrt((pow(netstat_1 - u, 2) + pow(netstat_2 - u, 2)) / 2)
    print gfi
